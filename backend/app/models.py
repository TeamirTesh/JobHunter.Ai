...
class User(db.Model):
    ...
    password = db.Column(db.String(255), nullable=True)  # null for OAuth-only users
...
class Application(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    company = db.Column(db.String(100), nullable=True)   # null when not known (e.g. from email)
    role = db.Column(db.String(100), nullable=True)     # null when not known
    company_domain = db.Column(db.String(255), nullable=True)  # e.g. wellsfargo.com for Clearbit logo
    ...
    def __repr__(self):
        c = self.company or 'Unknown'
        r = self.role or 'Unknown'
        return f"<Application {c} - {r} ({self.status})>"
